...
plotMode = 1; % default is to plot the results with saccade detection & stats on velocity & acceleration traces
...
%% buttons for the GUI
% which plot to check
buttons.seeSaccades = uicontrol(fig,'string','head vel classification?','Position',[0,410,100,30],...
    'callback', 'plotMode = 0; processTrial; updateGUI;');
buttons.seeVOR = uicontrol(fig,'string','head velocity filtering','Position',[0,380,100,30],...
    'callback', 'plotMode = 1; processTrial; updateGUI;');
buttons.seeFiltering = uicontrol(fig,'string','head position filtering','Position',[0,350,100,30],...
