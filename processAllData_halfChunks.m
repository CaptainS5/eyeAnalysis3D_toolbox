...
for chunk = 1:8
    for i = height(all_user_info):-1:1 % i = 1:height(all_user_info) %
        %try
            ...
            % Each of them is the beginning of a new game, so the previous to
            % each of them is the end of the previous game
            ...
            if chunk == 1
                ...
                eyeTrial.headAligned = eyeTrial.headAligned(1:floor(size(eyeTrial.headAligned,1)/2), :);
                eyeTrial.eyeAligned = eyeTrial.eyeAligned(1:floor(size(eyeTrial.eyeAligned,1)/2), :);
            elseif chunk == 2
                eyeTrial.headAligned = eyeTrial.headAligned(1:zeroIndices(2)-1, :);
                eyeTrial.eyeAligned = eyeTrial.eyeAligned(1:zeroIndices(2)-1, :);
                eyeTrial.headAligned = eyeTrial.headAligned(floor(size(eyeTrial.headAligned,1)/2):end, :);
                eyeTrial.eyeAligned = eyeTrial.eyeAligned(floor(size(eyeTrial.eyeAligned,1)/2):end, :);
            elseif chunk == 3
            ...
            %% DANI April 17th --> Some stuff is breaking (e.g., participant 50, first file of first session)
            % Dot indexing is not supported for variables of this type.

            % Error in findVOR (line 24)
            % for ii = 1:size(gazeFix.onsetI, 1)
            
            % Error in processAllData_halfChunks (line 316)
            %            [eyeTrial.VOR eyeTrial.classID] = findVOR(eyeVelXY, headVelXY, eyeTrial.headTrace.rotVel3DFilt, ...

            % .gazeFix seems to be empty, which is weird ??
            % Im using this try.catch thing to at least make the rest of
            % the code work...
            ...
            try
                ...
                % Get indices of saccades that are not correct
                dur_sac = eyeTrial.saccade.offsetTime - eyeTrial.saccade.onsetTime;
                indices = (dur_sac <= 0.5 & dur_sac > 0);
    ...
    save(['data\post_processed\study2\S2-small-post-data-' num2str(chunk) '.mat'], 'small_all_user_info', '-v7.3');
end
