...
columnNames = {'UserID', 'Session', 'Day', 'Duration', 'ETDDC', 'Files', 'EyeTrial', 'Calibration'};
variableTypes = {'string', 'int8', 'string', 'double', 'logical', 'string', 'cell', 'cell'};
...
for userFileI = 1:length(fileName)
    ...
    if (isequal(userID, prevUserID) && ~isequal(day, prevDay))
        ...
        all_user_info = [all_user_info ; ...
            ...
            eyeTrial.headAligned.timestamp(end) - eyeTrial.headAligned.timestamp(1), ...
...
for i = 1:height(all_user_info)
    ...
    if ~isempty(discrepancies)
        fprintf('User %s Session %d - Array is not in ascending order. Discrepancies found at indices: \n', all_user_info.UserID(i), all_user_info.Session(i));
        disp(discrepancies);
    end
end
