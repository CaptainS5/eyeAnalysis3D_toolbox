...
% % note that here we switched the axis already--outdated, don't use

for fileI = 1:length(fileName)
    eyeTrial = [];

    %% sort head data
        % raw data is in meters and seconds, a right-handed x+right, y+up, and z+back coordinate system
    ...
    %         end
        ...
        headFrameData.ori_Qw = rawD.referenceFromBody_pose_rotation_z; % w; z
        headFrameData.ori_Qx = -rawD.referenceFromBody_pose_rotation_y; % z; y
        headFrameData.ori_Qy = -rawD.referenceFromBody_pose_rotation_w; % x; w
        headFrameData.ori_Qz = rawD.referenceFromBody_pose_rotation_x; % y; x
